...
import com.app.drylining.chat.ui.adapter.CheckboxUsersAdapter;
...
public class SelectUsersActivity extends BaseActivity {
    ...
    List<QBUser> m_userList = new ArrayList<>();
    List<QBUser> m_searchUserList = new ArrayList<>();
    QBUser m_currentUser;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        m_currentUser = QBChatService.getInstance().getUser();
    ...
    private void loadUsersFromQb() {
        ...
        QBUsers.getUsersByTags(tags, null).performAsync(new QBEntityCallback<ArrayList<QBUser>>() {
            ...
            public void onSuccess(ArrayList<QBUser> result, Bundle params) {
                ...
                usersAdapter = new CheckboxUsersAdapter(SelectUsersActivity.this, m_userList);
                if (dialog != null) {
    ...
    private void updateUserList() {
        ...
        if(searchText.equals("")) {
            m_searchUserList.clear();
            m_searchUserList.addAll(m_userList);
        }
        else {
            m_searchUserList.clear();
            for(int i = 0; i < m_userList.size(); i++) {
                QBUser user = m_userList.get(i);
                String name = user.getFullName().toLowerCase();
                if(name.contains(searchText)) {
                    m_searchUserList.add(user);
                }
            }
        }

        usersAdapter = new CheckboxUsersAdapter(SelectUsersActivity.this, m_searchUserList);
        usersListView.setAdapter(usersAdapter);
    }
    ...
    protected boolean isUserMe(QBUser user) {
        return m_currentUser != null && m_currentUser.getId().equals(user.getId());
    }
}

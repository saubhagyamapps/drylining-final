...
import com.app.drylining.chat.async.BaseAsyncTask;
import com.app.drylining.chat.ui.dialog.ProgressDialogFragment;
import com.app.drylining.chat.utils.ImageUtils;
import com.app.drylining.chat.utils.constant.SchemeType;
...
public class GetFilepathFromUriTask extends BaseAsyncTask<Intent, Void, File> {
    ...
    public File performInBackground(Intent... params) throws Exception {
        ...
        if (SchemeType.SCHEME_CONTENT.equalsIgnoreCase(uriScheme) && !isFromGoogleApp && !isKitKatAndUpper) {
            ...
            Cursor cursor = DRYLINING.getInstance().getContentResolver().query(uri, filePathColumn, null, null, null);
            if (cursor != null) {
